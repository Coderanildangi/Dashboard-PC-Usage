            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        usageData.MachineName = reader["machineName"].ToString();
                        usageData.CPUUsage = Math.Round((double)reader["CPUUsage"], 2);
                        usageData.RAMUsage = Math.Round((double)reader["RAMUsage"], 2);
                        usageData.DiskUsage = Math.Round((double)reader["DiskUsage"], 2);
